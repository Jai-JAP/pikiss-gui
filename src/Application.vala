...
public class PiKISS : Window {
    private void setup_apps(string directory, string category, Box box) {
        try {
            ...
            string? name = null;
            string pikiss = Config.pikissdir;
            string sharedir = Config.PACKAGE_SHAREDIR + "/" + Config.PROJECT_NAME + "/";
            string desc;
            while ((name = dir.read_name ()) != null) {
                ...
                if (FileUtils.test (path, FileTest.IS_EXECUTABLE)) {
                    try {
                        string cmd = sharedir + "getdesc.sh " + name.replace (".sh", "").replace ("VSCode", "VSCode/ium") ;
                        Process.spawn_command_line_sync (cmd, out desc);
                    } catch (GLib.SpawnError e) {
                        warning ("Error! getdesc.sh spawn error: " + e.message);
                    }

				    var button = new Button.with_label (name.replace (".sh", "").replace ("VSCode", "VSCode/ium"));
                    button.set_tooltip_text(desc);
				    button.set_size_request(80, 32);
				    button.clicked.connect (() => {
	                    try {
	                        GLib.AppInfo.create_from_commandline ("x-terminal-emulator -e bash -c 'cd \"" + pikiss + "\" && \"" + directory + "/" + button.label.replace ("VSCode/ium", "VSCode") + ".sh\"'", null, GLib.AppInfoCreateFlags.NONE).launch (null,null);
	                    } catch (GLib.Error e) {
